...
export function lboModel(state, base) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    // cash sweep: spare cash repays debt; a shortfall drains the cash balance
    // first and then draws on the revolver, so it cannot vanish.
    const pay = Math.min(Math.max(fcf, 0), beg);
    let end = beg - pay, draw = 0;
    const spare = fcf - pay;
    if (spare < 0) {
      const fromCash = Math.min(cashAcc, -spare);
      cashAcc -= fromCash; draw = -spare - fromCash; end += draw;
    } else cashAcc += spare;
    yrs.push({ ebitda, da, ebit, int_, ebt, tax, ni, fcf, beg, pay, draw, end, cashAcc });
    beg = end;
  }
  const exitEbitda = f.ebitda[4], exitEV = exitEbitda * a.lboExit;
  const exitEq = exitEV - yrs[4].end + yrs[4].cashAcc;
  const mom = sponsor > 0 ? Math.max(exitEq, 0) / sponsor : 0;
  const irr = sponsor > 0 ? (mom > 0 ? Math.pow(mom, 1 / 5) - 1 : -1) : 0;
