...
/**
 * Fundamental (funded) growth — the consistency check most models skip.
 *
 *   growth = reinvestment rate x return on invested capital
 *
 * Growth isn't free: a company has to put capital back into the business and earn a
 * return on it. This works out what growth the company's OWN capital allocation
 * supports, so an assumed growth rate can be checked against it rather than taken
 * on faith. Returns null when the inputs can't support the calculation.
 */
export function fundamentals(state, assumedGrowth) {
  const h = state.hist, a = state.asm;
  const L = h.rev.length - 1, P = L - 1;
  if (L < 1) return null;                                   // need 2 years for the NWC change

  const ebit = h.rev[L] - h.cogs[L] - h.sga[L];
  const nopat = ebit * (1 - a.taxRate);
  const investedCapital = h.debt[L] + h.eq[L] - h.cash[L];
  if (!(nopat > 0) || !(investedCapital > 0)) return null;   // loss-making or net-cash: ratio is meaningless

  const roic = nopat / investedCapital;
  const nwc = (i) => h.ar[i] + h.inv[i] - h.ap[i];
  const reinvestment = h.capex[L] - h.da[L] + (nwc(L) - nwc(P));
  const reinvestRate = reinvestment / nopat;
  const fundedGrowth = reinvestRate * roic;

  // what the assumed growth would actually require
  const g = typeof assumedGrowth === "number" && isFinite(assumedGrowth) ? assumedGrowth : a.growth[0];
  const requiredRate = roic > 0 ? g / roic : null;
  const requiredAmount = requiredRate == null ? null : requiredRate * nopat;

  let verdict = "supported";
  if (g > fundedGrowth + 0.005) verdict = requiredRate != null && requiredRate > 1 ? "unfundable" : "stretch";
  else if (g < fundedGrowth - 0.005) verdict = "conservative";

  return {
    nopat, investedCapital, roic, reinvestment, reinvestRate, fundedGrowth,
    assumedGrowth: g, requiredRate, requiredAmount, verdict,
    gap: g - fundedGrowth,
  };
}
